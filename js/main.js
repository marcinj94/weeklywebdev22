...
function addTextContent(title, description) {
  clientTitle.textContent = title;
  clientDescription.textContent = description;
}
...
function clientChoice() {
  ...
  if (this.dataset.client == "1") {
    addTextContent("Anthony Green", "Web developer");
  } else if (this.dataset.client == "2") {
    addTextContent("Kyle Roberts", "Company CEO");
  } else if (this.dataset.client == "3") {
    addTextContent("Bella Morgan", "Project manager");
  }
}
clientImages.forEach(clientImage => {
...
$doc.on("scroll", function() {
  ...
  const $servicesOffset = $services.offset().top;
  const $headerHeight = $header.outerHeight();

  if ($scrollValue < $servicesOffset) {
    $header.css({
      filter: `brightness(${1 - $scrollValue / $headerHeight})`
    });
  }
});
